...
TEST_YEARS = 5  # length of the held-out evaluation window (matches every model script)
...
def lake_scale() -> tuple[float, float]:
    """Return (dynamic range, std) of the lake level over the recent test window."""
...
def main() -> None:
    ...
    # Restrict every model to the same recent test window for an apples-to-apples comparison.
    test_cutoff = preds["date"].max() - pd.DateOffset(years=TEST_YEARS)
    ...
    lake_range, lake_std = lake_scale()
